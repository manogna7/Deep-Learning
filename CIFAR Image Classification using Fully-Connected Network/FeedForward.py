...
class SigmoidCrossEntropy:
    ...
    def forward(self, logits, labels):
        ...
        self.loss = -np.mean(labels * np.log(self.sigmoid_output + 1e-9) + 
...
class ReLU:
    ...
    # TODO: No parameters so nothing to do during a gradient descent step
...
class LinearLayer:
    ...
    def backward(self, grad):
        ...
        self.grad_weights = (
            np.dot(self.input.T, grad) / batch_size
        )  # TODO batch normalization?
        self.grad_bias = (
            np.sum(grad, axis=0, keepdims=True) / batch_size
        )  # TODO batch normalization?
...
######################################################
    def step(self, step_size, momentum=0.8, weight_decay=0.0):
        ...
        self.velocity_bias = momentum * self.velocity_bias - step_size * (
            self.grad_bias + weight_decay * self.bias
        )
...
def evaluate(model, X_val, Y_val, batch_size):
    ...
    for i in range(0, num_examples, batch_size):
        ...
        error_rate = 1 - (np.sum(predictions == Y_batch) / len(predictions))
...
def main():
    ...
    net = FeedForwardNeuralNetwork(
        input_dim, output_dim, width_of_layers, number_of_layers
    )
    ...
    for i in range(max_epochs):
        ...
        logging.info(
            "[Epoch {:3}]   Loss:  {:8.4}     Train Acc:  {:8.4}%      Val Acc:  {:8.4}%".format(
                i, epoch_avg_loss, epoch_avg_acc, tacc * 100
            )
        )
    ...
    ax1.plot(
        [np.ceil((i + 1) * num_batches) for i in range(len(val_losses))],
        val_losses,
        c="red",
        label="Val. Loss",
    )
    ...
    ax2.plot(
        [np.ceil((i + 1) * len(X_train) / batch_size) for i in range(len(val_accs))],
        val_accs,
        c="blue",
        label="Val. Acc.",
    )
